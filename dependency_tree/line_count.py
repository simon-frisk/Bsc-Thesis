...
def package_line_counts():
    ...
    git_folders = os.listdir('../git')
    code_lines = {}
    for package_name in packages:
        if package_name not in git_folders:
            print(f"Skipping {package_name}: Not downloaded")
        results = subprocess.run(["cloc", f"../git/{package_name}", '--csv'], capture_output=True, text=True)
...
def line_count_violin_plot(code_lines_dict):
    plt.violinplot(code_lines_dict.values(), showmeans=True)
    plt.show()
